...
module.exports = function() {
    let firstTripHouseList = [ { x: 0, y: 0 } ];
    let santaHouseList = [ { x: 0, y: 0 } ];
    let roboHouseList = [ { x: 0, y: 0 } ];
    ...
    directions.forEach(dir => {
        let lastHouse = santasTurn 
        ? santaHouseList[santaHouseList.length - 1] 
        : roboHouseList[roboHouseList.length - 1]
        let nextHouse;
        
        if (dir == "^") {
            nextHouse = { x: lastHouse.x, y: lastHouse.y+1 }
        }
        if (dir == "v") {
            nextHouse = { x: lastHouse.x, y: lastHouse.y-1 }
        }
        if (dir == ">") {
            nextHouse = { x: lastHouse.x+1, y: lastHouse.y }
        }
        if (dir == "<") {
            nextHouse = { x: lastHouse.x-1, y: lastHouse.y }
        }
        ...
        if (santasTurn) {
            santaHouseList.push(nextHouse)
        } else {
            roboHouseList.push(nextHouse)
        }
        santasTurn = !santasTurn;
        
        
        // First Trip
        lastHouse = firstTripHouseList[firstTripHouseList.length - 1]
        ...
        firstTripHouseList.push(nextHouse)
    })


    const totalCount = [... new Set(firstTripHouseList.map(house => `${house.x}|${house.y}`))].length
    const santaRoboCount = [... new Set(santaHouseList.concat(roboHouseList).map(house => `${house.x}|${house.y}`))].length

    return [
        "Total houses visited: " + totalCount,
        "Combined houses visited: " + santaRoboCount
    ]
}